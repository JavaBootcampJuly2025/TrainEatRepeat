...
class AccountServiceTest {
    ...
    @BeforeEach
    void setUp() {
        validRegisterRequestMale = new RegisterRequest(
                "testuser",
                "test@example.com",
                "password123",
                "password123",
        ...
        validRegisterRequestFemale = new RegisterRequest(
                "testuser2",
                "test2@example.com",
                "PassworD123!",
                "PassworD123!",
    ...
    void registerUser_Success() {
        ...
        when(passwordEncoder.encode(validRegisterRequestFemale.password())).thenReturn("encodedPassword");
    ...
    @Test
    @DisplayName("Should calculate BMI and BMR correctly for both male and female users during registration")
    void registerUser_CalculatesBmiAndBmrCorrectlyForBothGenders() {
        ...
        when(passwordEncoder.encode(validRegisterRequestMale.password())).thenReturn("encodedPasswordMale");

        when(userRepository.findByEmail(validRegisterRequestFemale.email())).thenReturn(null);
        when(passwordEncoder.encode(validRegisterRequestFemale.password())).thenReturn("encodedPasswordFemale");
        ...
        UserDTO savedMaleUser = savedUsers.stream()
        ...
        UserDTO savedFemaleUser = savedUsers.stream()
        ...
        float expectedBmiMale = validRegisterRequestMale.weight() /
        ...
        float expectedBmrMale = (10 * validRegisterRequestMale.weight()) +
                (6.25f * validRegisterRequestMale.height()) -
                (5 * validRegisterRequestMale.age()) + 5;
        assertEquals(expectedBmrMale, savedMaleUser.getBmr(), 0.1, "BMR should be correct for male");

        float expectedBmiFemale = validRegisterRequestFemale.weight() /
                (float) Math.pow(validRegisterRequestFemale.height() / 100.0, 2);
        assertEquals(expectedBmiFemale, savedFemaleUser.getBmi(), 0.01, "BMI should be correct for female");

        float expectedBmrFemale = (10 * validRegisterRequestFemale.weight()) +
                (6.25f * validRegisterRequestFemale.height()) -
                (5 * validRegisterRequestFemale.age()) - 161;
        assertEquals(expectedBmrFemale, savedFemaleUser.getBmr(), 0.1, "BMR should be correct for female");
    }
