...
class MealServiceTest {
  ...
  @Mock private MealRecordConverter mealRecordConverter;
  @Mock private UsdaClient usdaClient;
  ...
  void setUp() {
    ...
    savedMealDTO =
        buildMealDTO(
            UUID.randomUUID().toString(), "Apple", 78.0f, 20.7f, 0.45f, 0.3f, 150.0f, TODAY);
    expectedMealRecordResponse =
        buildMealRecordResponse(
            savedMealDTO.getId(),
    ...
    lenient()
        .when(mealRecordConverter.toResponse(any(MealDTO.class)))
        .thenReturn(expectedMealRecordResponse);
  }
  ...
  void submitMeal_FoodNotFound_Throws() {
    when(usdaClient.searchFood(anyString()))
        .thenReturn(new UsdaSearchResponse(Collections.emptyList()));

    TrainEatRepeatException ex =
        assertThrows(
  ...
  void submitMeal_NoNutritionalInfo_Throws() {
    UsdaSearchResponse usdaResponseNoNutrients =
        new UsdaSearchResponse(
            List.of(
                new UsdaSearchResponse.Food(
                    "FoodWithoutNutrients",
                    null,
                    null,
                    null,
                    null,
  ...
  void getMealsForUser_NoMealsFound_Throws() {
    ...
    TrainEatRepeatException ex =
        assertThrows(
            TrainEatRepeatException.class, () -> mealService.getMealsForUser(TEST_UUID, null));
  ...
  void deleteMealById_NotFound() {
    ...
    TrainEatRepeatException exception =
        assertThrows(TrainEatRepeatException.class, () -> mealService.deleteMealById(id));
  ...
  void updateMeal_Success() {
    ...
    when(mealRecordRepository.save(any(MealDTO.class)))
        .thenAnswer(invocation -> invocation.getArgument(0));

    MealRecordResponse updatedExpectedResponse =
        buildMealRecordResponse(mealId, "Banana", 78.0f, 20.7f, 0.45f, 0.3f, 150.0f, TODAY);
  ...
  void getNutritionStatistics_WithMeals() {
    ...
    List<MealDTO> mealDTOs =
        List.of(
            buildMealDTO(
                UUID.randomUUID().toString(), "Meal1", 100f, 10f, 5f, 2f, 200f, startOfWeek),
            buildMealDTO(
    ...
    UserNutritionStatisticsResponse stats =
        mealService.getNutritionStatistics(TEST_UUID, TimePeriod.WEEK);
  ...
  private MealRecordResponse buildMealRecordResponse(
      String id,
      String foodName,
      float calories,
      float carbs,
